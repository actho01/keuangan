...
import DeletePengeluaranApi from '@/api/laporan/pengeluaran/DeletePengeluaran';
import EditPengeluaran from '@/app/components/molekules/EditPengeluaran';
import GetAllPengeluaranOperasionalApi from '@/api/laporan/pengeluaran/GetAllPengeluaranOperasional';
...
const LaporanPengeluaranHarian = () => {
  ...
  const fetchPengeluaran = async () => {
    try {
      const res = await GetAllPengeluaranOperasionalApi();
      console.log('res', res);
      if (res.status) {
  ...
  const handleDelete = async (id) => {
    try {
      const res = await DeletePengeluaranApi({ id });
      if (res.status) {
  ...
  const totalPages = Math.ceil((pengeluaran?.length ?? 1) / itemsPerPage);
  ...
  return (
    <UserTemplate page={'Laporan Pengeluaran Operasional'}>
      <div className="p-4">
            ...
            <Button
              ...
              onClick={() =>
                (window.location.href =
                  '/laporan/pengeluaran/operasional/tambah')
              }
            />
          </fieldset>
        </section>
        {loading ? (
          ...
          <p className="text-center text-red-500">
            Tidak ada data laporan pengeluaran
          </p>
        ) : (
