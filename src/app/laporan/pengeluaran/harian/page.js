...
import GetAllPengeluaranHarianApi from '@/api/laporan/pengeluaran/GetAllPengeluaranHarianApi';
...
const LaporanPengeluaranHarian = () => {
  ...
  const fetchPengeluaran = async () => {
    try {
      const res = await GetAllPengeluaranHarianApi();
      console.log('res', res);
      if (res.status) {
  ...
  return (
    <UserTemplate page={'Laporan Pengeluaran Harian'}>
      <div className="p-4">
        <section className="flex flex-col items-end space-y-3">
          ...
          <fieldset className="w-1/6 ">
            <Button
              ...
              onClick={() =>
                (window.location.href = '/laporan/pengeluaran/harian/tambah')
              }
            />
          </fieldset>
        </section>
