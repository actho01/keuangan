'use client';
import AddPemasukanApi from '@/api/laporan/pemasukan/AddPemasukanApi';
...
const TambahPemasukan = () => {
  ...
  const handleTambahPemasukan = async (event) => {
    event.preventDefault();
    setIsSubmitting(true);

    try {
      const res = await AddPemasukanApi({ kategori, deskripsi, value });

      if (res.status) {
        setAlertSeverity('success');
        setShowAlert(true);
        setAlertMessage('Pemasukan added successfully');
        setKategori('Finance');
        setDeskripsi('');
        setValue('');
      } else {
  ...
  return (
    <UserTemplate page={'Tambah Laporan Pemasukan'}>
      <section className="p-4">
        <form
          onSubmit={handleTambahPemasukan}
